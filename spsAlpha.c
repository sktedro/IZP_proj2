...
bool reallocRows(tab_t *tab, int by){
  int newRowN = tab->len + by;
  if(by > 0){
    row_t *p = realloc(ROW, newRowN*sizeof(row_t));
    if(!p) 
      return false;
    ROW = p;
    tab->len = newRowN;
    CELL(newRowN - 1) = malloc(sizeof(cell_t));
    if(!CELL(newRowN - 1)) 
      return false;
    SROW(newRowN - 1).len = 1;
    CONT(newRowN - 1, 0) = malloc(1);
    if(!CONT(newRowN - 1, 0))
      return false;
    SCELL(newRowN - 1, 0).len = 1;
  }else if(by < 0){
